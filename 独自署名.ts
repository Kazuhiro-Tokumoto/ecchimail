...
private readonly P = 0xFFFFFFFDFFFFFFFFFFFFFFFFFFFFFFFFn;
private readonly A = 0xFFFFFFFDFFFFFFFFFFFFFFFFFFFFFFFCn; // p-3
private readonly B = 0xE87579C11079F43DD824993C2CEE5ED3n;
private readonly N = 0xFFFFFFFE0000000075A30D1B9038A115n;
private readonly G: [bigint, bigint] = [
    0x161FF7528B899B2D0C28607CA52C5B86n,
    0xCF5AC8395BAFEB13C02DA292DDED7A83n,
];
  ...
  private readonly _shifts: bigint[] = (() => {
    const s: bigint[] = [];
    for (let i = 0; i < 128; i += 8) s.push(BigInt(i));
    return s;
  })();
...
public sign(
    ...
    const k = this.generateK(message, BigintToBytes(privKeyBigint)) % this.N;
    ...
    if (e === 0n || s === 0n) {
        throw new Error("Signature generation failed: e or s is zero");
    }
    return [
        BigintToBytes(R[0]),
        BigintToBytes(R[1]),
...
public verify(
    ...
    const pubKeyBigint: [bigint, bigint] = [
        bytesToBigInt(pubKey[0]),
        bytesToBigInt(pubKey[1]),
    ];
    const R: [bigint, bigint] = [
        bytesToBigInt(signature[0]),
        bytesToBigInt(signature[1]),
    ];
    const s = bytesToBigInt(signature[2]);
    const e = bytesToBigInt(
        this.sha256(
            this.concat(
                BigintToBytes(R[0]),
  ...
  private BigintToBytes(n: bigint, byteLen = 16): Uint8Array {
...
/*
export class PointPairSchnorrP256 {
private readonly P = 0xe090965e453a25f3n;
private readonly A = 0xe090965e453a25f0n; // p-3
private readonly B = 0x863df0d6e4c7c56fn;
private readonly N = 0xe090965e2eec8e2dn;
private readonly G: [bigint, bigint] = [
    0x15f08849b7eff82n,
    0x3f3be254999d76n,
];
  ...
  private readonly _shifts: bigint[] = (() => {
    const s: bigint[] = [];
    for (let i = 0; i < 64; i += 8) s.push(BigInt(i));
    return s;
  })();
...
public verify(
  ...
  private BigintToBytes(n: bigint, byteLen = 8): Uint8Array {
    const b = new Uint8Array(byteLen);
    for (let i = byteLen - 1; i >= 0; i--) {
      b[i] = Number(n & 0xffn);
      n >>= 8n;
 ...
 /*
 export class PointPairSchnorrP256 {
private readonly P = 0xba13b221n;
private readonly A = 0xba13b21en; // p-3
private readonly B = 0xa7404462n;
private readonly N = 0xba13a3a1n;
private readonly G: [bigint, bigint] = [
    0x70ad66bcn,
    0x4531b7bdn,
];
  ...
  private readonly _shifts: bigint[] = (() => {
    const s: bigint[] = [];
    for (let i = 0; i < 32; i += 8) s.push(BigInt(i));
    return s;
  })();
...
public verify(
  ...
  private BigintToBytes(n: bigint, byteLen = 4): Uint8Array {
    const b = new Uint8Array(byteLen);
    for (let i = byteLen - 1; i >= 0; i--) {
      b[i] = Number(n & 0xffn);
      n >>= 8n;
 ...
 /*
 export class PointPairSchnorrP256 {
private readonly P = 0xdc3fn;
private readonly A = 0xdc3cn; // p-3
private readonly B = 0xa8ecn;
private readonly N = 0xdc85n;
private readonly G: [bigint, bigint] = [
    0xc8e5n,
    0x5ae4n,
];
  ...
  private readonly _shifts: bigint[] = (() => {
    const s: bigint[] = [];
    for (let i = 0; i < 16; i += 8) s.push(BigInt(i));
    return s;
  })();
...
public verify(
  ...
  private BigintToBytes(n: bigint, byteLen = 2): Uint8Array {
    const b = new Uint8Array(byteLen);
    for (let i = byteLen - 1; i >= 0; i--) {
      b[i] = Number(n & 0xffn);
      n >>= 8n;
  ...
  */
 export class PointPairSchnorrP256 {
private readonly P = 0x95n;
private readonly A = 0x92n; // p-3
private readonly B = 0x26n;
private readonly N = 0x89n;
private readonly G: [bigint, bigint] = [
    0x7dn,
    0x15n,
];
  ...
  private readonly _shifts: bigint[] = (() => {
    const s: bigint[] = [];
    for (let i = 0; i < 8; i += 8) s.push(BigInt(i));
    return s;
  })();
...
public verify(
  ...
  private BigintToBytes(n: bigint, byteLen = 1): Uint8Array {
    const b = new Uint8Array(byteLen);
    for (let i = byteLen - 1; i >= 0; i--) {
      b[i] = Number(n & 0xffn);
      n >>= 8n;
